...
describe("ClientService", () => {
  ...
  it("getAllClients filtra por owner cuando el usuario es asesor", async () => {
    ...
    expect(result.clients[0].createdAt).toBe(
      sampleClient().createdAt.toISOString()
    );
  });
  ...
  it("updateClient cambia estado y notifica el cambio", async () => {
    ...
    expect(clientRepositoryMock.update).toHaveBeenCalledWith("client-1", {
      status: "Activo",
    });
    expect(emailServiceMock.sendClientStatusChanged).toHaveBeenCalledWith(
      updated,
      "Contactado"
    );
