...
jest.mock(
  "../../../../src/modules/auth/repositories/refresh-token.repository",
  () => ({
...
describe("AuthService", () => {
  ...
  it("logout revoca todos los tokens del usuario", async () => {
    ...
    expect(refreshTokenRepositoryMock.revokeAllUserTokens).toHaveBeenCalledWith(
      "user-1"
    );
  });
  ...
  it("getCurrentUser lanza notFound cuando el usuario no existe", async () => {
    ...
    await expect(service.getCurrentUser("missing")).rejects.toEqual(
      errors.notFound("Usuario")
    );
  });
});
