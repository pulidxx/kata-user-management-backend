...
describe('UserService', () => {
  ...
  it('getAllUsers devuelve usuarios transformados', async () => {
    userRepositoryMock.findAll.mockResolvedValue([sampleUser({ role: 'admin' })]);
  ...
  it('deleteUser permite reasignar clientes antes de eliminar', async () => {
    ...
    userRepositoryMock.findById.mockResolvedValueOnce(sampleUser()).mockResolvedValueOnce(targetUser);
    ...
    expect(clientRepositoryMock.updateOwner).toHaveBeenCalledWith('user-1', 'target-user');
